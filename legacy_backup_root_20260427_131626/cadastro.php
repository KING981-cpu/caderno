...
<div class="container">
    ...
    <form action="salvar.php" method="POST" id="formMovimentacao" autocomplete="off">
        <label>Tipo:</label>
...
<script>
    ...
    let localidades = [<?php foreach($pdo->query("SELECT id_localidade, nome FROM localidade") as $l) { echo "{id:'".$l['id_localidade']."', nome:'".addslashes($l['nome'])."'},"; } ?>];
    let usuarios = [<?php foreach($pdo->query("SELECT id_usuario, nome FROM usuario") as $u) { echo "{id:'".$u['id_usuario']."', nome:'".addslashes($u['nome'])."'},"; } ?>];
    ...
    function salvarRapido(tabela, nomeInputId, txtInputId, hiddenId, modalId) {
        ...
        // --- VALIDAÇÃO DE DUPLICADO NO FRONT-END ---
